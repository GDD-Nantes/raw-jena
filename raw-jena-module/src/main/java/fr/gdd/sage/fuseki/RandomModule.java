 ...
 * work with `addModule`.
 */
public class RandomModule implements FusekiModule {
    Logger logger = LoggerFactory.getLogger(RandomModule.class);
    ...
    public void serverBeforeStarting(FusekiServer server) {
        ...
        for (var dap : server.getDataAccessPointRegistry().accessPoints()) {
            if (DatabaseMgr.isTDB2(dap.getDataService().getDataset())) {
                ...
                QC.setFactory(ds.getContext(), new OpExecutorRandom.OpExecutorRandomFactory(sageContext));
            }
